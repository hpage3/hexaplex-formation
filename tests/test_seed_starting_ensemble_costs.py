...
def test_cost_audit_runs_on_small_synthetic_input(tmp_path):
    ...
    for unit in [6, 7]:
        for index in range(3):
            ...
            rows.append(synthetic_row(unit, "axially_misregistered", index, 0.28 + index * 0.01))
...
def test_cost_audit_includes_axially_misregistered_class(tmp_path):
    rows = []
    for index in range(2):
        rows.append(synthetic_row(6, "formed_perturbed", index, index * 0.01))
        rows.append(synthetic_row(6, "loose_initial", index, 0.2 + index * 0.01))
        rows.append(synthetic_row(6, "axially_misregistered", index, 0.32 + index * 0.01))
    input_csv = tmp_path / "seed.csv"
    write_csv(input_csv, rows)
    ...
    with (tmp_path / "summary.csv").open(newline="", encoding="utf-8") as handle:
        summary_rows = list(csv.DictReader(handle))
    assert {row["start_class"] for row in summary_rows} == {"loose_initial", "axially_misregistered"}
    assert any(row["register_phase_cost"] for row in summary_rows if row["start_class"] == "axially_misregistered")
...
def test_report_contains_axial_misregistration_language(tmp_path):
    rows = []
    for index in range(2):
        rows.append(synthetic_row(6, "formed_perturbed", index, index * 0.01))
        rows.append(synthetic_row(6, "loose_initial", index, 0.2 + index * 0.01))
        rows.append(synthetic_row(6, "radially_separated", index, 0.3 + index * 0.01))
        rows.append(synthetic_row(6, "angular_randomized_loose_initial", index, 0.35 + index * 0.01))
        rows.append(synthetic_row(6, "axially_misregistered", index, 0.32 + index * 0.01))
    input_csv = tmp_path / "seed.csv"
    write_csv(input_csv, rows)
    ...
    report = (tmp_path / "report.md").read_text(encoding="utf-8")
    assert "synthetic register-perturbation proxy" in report
    assert "Axial-Misregistration Readout" in report
    assert "not as a physical trajectory" in report
