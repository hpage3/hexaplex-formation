#!/usr/bin/env python3
"""Inventory Nick's confirmed HXC590 16-mer simulation input."""
...
DEFAULT_COPIED = Path("inputs/candidates/nick_16mer_hexaplex_antiparallel_30deg_ideal.pdb")
DEFAULT_EXISTING_PROFILE = Path("outputs/mini_hexaplex/radial_profiles/full_length_baseline_radial.csv")
DEFAULT_PROFILE = Path("outputs/metrics/hxc590_s1_nick_16mer_antiparallel_30deg_profile.csv")
DEFAULT_INVENTORY = Path("outputs/metrics/hxc590_s1_nick_16mer_candidate_inventory.csv")
DEFAULT_REPORT = Path("outputs/reports/hxc590_s1_nick_16mer_candidate_inventory_report.md")
...
def build_inventory_row(args: argparse.Namespace) -> dict[str, str]:
    if not args.source_pdb.exists():
        raise FileNotFoundError(f"Nick-confirmed source PDB not found: {args.source_pdb}")
    ...
    metadata = {
        "candidate_label": "nick_16mer_antiparallel_30deg_ideal",
        ...
        "caution": "Nick clarified this PDB is the 16-mer simulation input. This profile is used for a compatibility screen, not a unique refined phase assignment.",
    }
    ...
    return {
        ...
        "provenance_note": "Nick clarified that Hexaplex_AntiParallel_30deg_Ideal.pdb is the 16-mer simulation input.",
    }
