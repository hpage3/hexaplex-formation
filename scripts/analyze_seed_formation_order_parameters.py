...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--loose-mode",
        choices=["preserved_angular", "angular_randomized", "radially_separated", "axially_misregistered", "all"],
        default="all",
    )
...
def generate_axially_misregistered(reference: SeedReference, sample_index: int, rng: random.Random) -> list[PDBAtom]:
    """Loose class that preserves rough phase while adding stack-axis register offsets."""

    base_atoms = generate_loose_initial(reference, sample_index, rng)
    grouped = group_by_chain(base_atoms)
    axis = normalize_vector(reference.axis)
    step = rng.uniform(2.8, 4.4)
    offset_levels = [-2.5, 1.5, -0.5, 0.5, -1.5, 2.5]
    atoms: list[PDBAtom] = []
    for chain_index, (chain_id, chain_atoms) in enumerate(grouped.items()):
        jitter = rng.uniform(-0.35, 0.35)
        axial_offset = (offset_levels[chain_index % len(offset_levels)] + jitter) * step
        translation = (axis[0] * axial_offset, axis[1] * axial_offset, axis[2] * axial_offset)
        atoms.extend(transform_chain(chain_atoms, rotation_matrix_from_axis_angle(axis, 0.0), translation))
    return atoms
...
def loose_modes_to_generate(loose_mode: str) -> list[str]:
    if loose_mode == "all":
        return ["preserved_angular", "angular_randomized", "radially_separated", "axially_misregistered"]
    return [loose_mode]
...
def generate_loose_atoms_for_mode(
    ...
    if mode == "axially_misregistered":
        return (
            "axially_misregistered",
            f"central{reference.unit_count}_axially_misregistered_{sample_index:04d}",
            generate_axially_misregistered(reference, sample_index, rng),
        )
    return (
        "loose_initial",
...
def write_report(
    ...
    lines = [
        ...
        "",
        "The `axially_misregistered` class is a synthetic register-perturbation proxy. It asks whether chain offsets along the stack axis produce a distinct cost signature from radial separation.",
        "",
